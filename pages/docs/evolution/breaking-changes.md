...
## `6.4` The scope of this policy

### `6.5` In scope

`6.5.1` Language: syntax, static checks, execution
    semantics of language constructs.

`6.5.2` The interop subsystem of the language: how
    Java declarations are seen from Kotlin, and how Kotlin declarations
    are seen from Java.

`6.5.3` Compatibility of binary artifacts produced by
 kotlinc with one another and with Java binaries

`6.5.4` Standard library: API and contracts of the
    declarations in kotlin-stdlib (and its  extensions such as for JRE
    7 & 8).

`6.5.5` CLI parameters of the compiler except for the
    -X keys.

`6.5.6` Language Feature, Syntax & API Migration
 tools.
...
`6.6.1` Build tools and plugins for them (e.g. Gradle
    support).

`6.6.2` IDE and static analysis tools (other than the
    compiler).

`6.6.3` Java2Kotlin converter and other source code
    manipulation tools.

`6.6.4` Experimental language features & APIs.

`6.6.5` APIs and contracts of libraries other than the
    standard library.

`6.6.6` API of the compiler.

`6.6.7` Scripting support and Compiler REPL loop.

`6.6.8` dokka (docs generation tool).

`6.6.9` Internal packages of the standard library.

`6.6.10` kotlin-reflect - until it graduates from the
    experimental status.

`6.6.11` kotlin-script-runtime - until it graduates
    from the experimental status.

## `6.7` Different requirements for different platforms
...
`6.9.1` Binary compatibility is not guaranteed for the
JS world, because everything is usually built from sources.

### `6.10` Kotlin/Native

`6.10.1` Until Kotlin/Native reaches an official
    release, no compatibility guarantees are given for Kotlin/Native.

`6.10.2` After Kotlin/Native reaches the official
...
## `6.11` Examples: issues & non-issues

Notation: NC — new compiler, OC — old compiler.

### `6.12` Binary Change, issues

`6.12.1` A binary produced from unchanged code with NC
...
### `6.13` Binary Change, non-issues

`6.13.1` A binary compiled with NC may be rejected by
    OC, when it relies on new language features unsupported in OC.

`6.13.2` A binary compiled against a newer version of
    kotlin-stdlib fails when an older version of kotlin-stdlib is
    supplied at runtime.

`6.13.3` Adding generic parameters to existing
    declarations does not change the ABI on the JVM (due to erasure).

`6.13.4` Changes to signatures of functions marked
    @InlineOnly are not changing the ABI on the JVM.

`6.13.5` Adding supertypes to existing library
classes/interfaces.

### `6.14` Source Change, issues

`6.14.1` Sources that compiled with OC don't compile
    with NC against the same classpath.

`6.14.2` Unchanged sources compile with NC, but their
    behavior changes in a way significant for contract-abiding use
    cases.

### `6.15` Source Change, non-issues

`6.15.1` Code compilable with NC fails to compile with
    OC (e.g. due to new language features supported in NC).

`6.15.2` The code breaks only if the user alters the
    build configuration or compiler settings explicitly (i.e. in
    addition to advancing the compiler version).

### `6.16` Library Change, issues

`6.16.1` Making a contract on existing API more strict
    than it used to be in a previous version.

### `6.17` Library Change, non-issues

`6.17.1` Relaxing a contract on existing APIs.

`6.17.2` Clarification for unspecified behaviors.

`6.17.3` Changes in hashCode() are not breaking
    changes.

`6.17.4` Changes in toString() on other than Boolean,
    Numeric, and String types are not breaking changes.

`6.17.5` Improper loading of two different versions of
    stdlib at runtime.


### `6.18` Performance changes
...
## `7` Examples of subtle issues

Banishing the changes listed in this section may pose significant
problems for language evolution.

`7.1` Changes in type inference and overload
    resolution algorithms.

`7.1.1` It's reasonable to assume that overloads of
...
`7.1.2` Some improvements in the language (such as
...
`7.1.3` some innocuous-looking changes in the source
...
`7.3` Moving classes from one JAR to another.
...
`7.4` Compatibility with the future.
...
`7.5` Compatibility of mental models.
...
`7.6` Interop compatibility vs improvements.
...
`8.1` Changes to this policy need to be approved by
    the Kotlin Language Committee.

`8.2` Any proposed change needs to be published as
